...
aminoacids = component_vis.data.load_aminoacids()
...
for rank, inits in models.items():
    sorted_inits, sorted_errors = component_vis.multimodel_evaluation.sort_models_by_error(inits, aminoacids.data)
...
fms_with_selected = {}
for rank, inits in models.items():
    fms_with_selected[rank] = component_vis.multimodel_evaluation.similarity_evaluation(inits[0], inits)
...
component_vis.visualisation.scree_plot(selected_models, aminoacids.data, metric="Fit", ax=axes[0])
component_vis.visualisation.scree_plot(selected_models, aminoacids.data, metric="Core consistency", ax=axes[1])
...
model_3comp = component_vis.postprocessing.postprocess(selected_models[3], dataset=aminoacids)
component_vis.visualisation.components_plot(model_3comp)
plt.show()

###############################################################################
fig, ax = plt.subplots(figsize=(3 * 1.6, 3), tight_layout=True)
component_vis.visualisation.percentage_variation_plot(model_3comp, aminoacids.data, method="both", ax=ax)
ax.set_yscale("log")
plt.show()
...
# 4 components
# ~~~~~~~~~~~~
model_4comp = component_vis.postprocessing.postprocess(selected_models[4], dataset=aminoacids)
component_vis.visualisation.components_plot(model_4comp)
plt.show()

###############################################################################
fig, ax = plt.subplots(figsize=(3 * 1.6, 3), tight_layout=True)
component_vis.visualisation.percentage_variation_plot(model_4comp, aminoacids.data, method="both", ax=ax)
ax.set_yscale("log")
