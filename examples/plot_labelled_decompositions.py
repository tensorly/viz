...
ComponentVis uses labelled decompositions, implemented by storing the factor matrices as Pandas DataFrames.
...
# ^^^^^^^^^^^^^^^^^
#
# We will set the backend to pytorch to demonstrate that ComponentVis also works with non-numpy backends.
...
# `issue <https://github.com/MarieRoald/componentvis/issues/new/choose>`_.
...
# but since our goal here is not data analysis, but demonstrating how ComponentVis visualises
...
cp_tensor_postprocessed = component_vis.postprocessing.postprocess(cp_tensor, aminoacids)
...
# The postprocessed decomposition has labels for the modes, which :meth:`component_vis.visualisation.components_plot`
...
# :meth:`component_vis.postprocessing.resolve_cp_sign_indeterminacy` for more information on how ComponentVis
