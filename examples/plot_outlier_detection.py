...
we use the name *slabwise SSE* in ComponentVis. For a third-order tensor, :math:`\mathcal{X}`, and its PARAFAC
...
def fit_parafac(X, num_components, num_inits=5):
    ...
    model = component_vis.multimodel_evaluation.get_model_with_lowest_error(model_candidates, X)
    return component_vis.postprocessing.postprocess(model, X)
...
component_vis.visualisation.components_plot(true_model)
plt.show()
...
X = component_vis.utils.cp_to_tensor(true_model)
...
component_vis.visualisation.component_comparison_plot({"True": true_model, "First attempt": first_attempt})
plt.show()

###############################################################################
# We see that some of the components are wrong, and to quantify this, we can look at the factor match score:

fms = component_vis.factor_tools.factor_match_score(true_model, first_attempt)
print(f"The FMS is {fms:.2f}")

###############################################################################
# Next, we can create a scatter plot where we plot the leverage and residuals for each sample.

component_vis.visualisation.outlier_plot(first_attempt, X)
plt.show()
...
component_vis.visualisation.component_comparison_plot({"True": sampled_true_cp, "Second attempt": second_attempt})
plt.show()
...
fms = component_vis.factor_tools.factor_match_score(sampled_true_cp, second_attempt)
print(f"The FMS is {fms:.2f}")
...
component_vis.visualisation.outlier_plot(second_attempt, sampled_X)
plt.show()
...
sampled_true_cp = index_cp_tensor(true_model, selected_samples, mode=0)
component_vis.visualisation.component_comparison_plot({"True": sampled_true_cp, "Third attempt": third_attempt})
plt.show()

###############################################################################
# Here, we see that we finally uncovered the true components. This is also apparent from the FMS

fms = component_vis.factor_tools.factor_match_score(sampled_true_cp, third_attempt)
print(f"The FMS is {fms:.2f}")
...
component_vis.visualisation.outlier_plot(third_attempt, sampled_X)
plt.show()
...
sampled_true_cp = index_cp_tensor(true_model, selected_samples, mode=0)
component_vis.visualisation.component_comparison_plot({"True": sampled_true_cp, "Fourth attempt": fourth_attempt})
plt.show()
...
# These methods are detailed in :meth:`component_vis.outliers.get_slabwise_sse_outlier_threshold`
# and :meth:`component_vis.outliers.get_slabwise_sse_outlier_threshold`.
